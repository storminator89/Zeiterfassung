...
function loadSavedData() {
    browser.storage.local.get(['apiUrl', 'username', 'password']).then((result) => {
        if (result.apiUrl) {
...
function sendTimeEntry(action) {
    browser.storage.local.get(['apiUrl', 'latestEntryId']).then((result) => {
        const apiUrl = result.apiUrl;
        ...
        .then(data => {
            if (action === 'createNewWorkEntry') {
                browser.storage.local.set({latestEntryId: data.data.id});
            }
